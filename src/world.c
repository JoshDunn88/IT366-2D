...
Entity* world_new_object_old()
{
    ...
    if (!self) {
        slog("failed to create world entity");
        return NULL;
    }
    ...
    self->scale = gfc_vector2d(1, 1);
    //self->sprite = gf2d_sprite_load_image("images/100x100_rect.png");
    self->collider = rect_collider_new(gfc_vector2d(0, 0), gfc_vector2d(500, 50)); //very dumb make this a parameter for better cleanup
